...
const { Connection, PublicKey, Keypair, Transaction, SystemProgram,
...
async function run() {
  ...
  if (bal < 0.005) { log({ event: 'low_balance' }); return; }

  // On-chain activity: self-transfer
  ...
  // Arcium program ping
  const pid = new PublicKey(process.env.ARCIUM_PROGRAM_ID || '3TysCyYXyWpqNXDnQiwA4C2KiMSxGmBbTJADtGwFVeLr');
  const info = await conn.getAccountInfo(pid);
  log({ event: 'arcium_ping', active: info !== null });
}
